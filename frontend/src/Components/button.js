function Button({ text, type, className, variant, icon }) {
  const commonClasses = `text-sm uppercase p-2 rounded-lg ${className}`;

  const typeClasses = type === "fill" 
    ? variant === "blue"
  ...
  const iconClasses = icon ? "flex items-center justify-center w-2/5 p-2" : "w-full";

  return (
    <button className={`${typeClasses} ${borderClass} ${commonClasses} ${iconClasses}`}>
      {icon ? (
